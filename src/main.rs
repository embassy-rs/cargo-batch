...
use cargo::util::{profile, CliError};
use std::env;
use std::sync::Arc;
...
fn main2(config: &mut Config) -> CliResult {
    ...
    let global_args = App::new("cargo-multi")
        .settings(&[
        ...
        .get_matches_from_safe(global_args)?;
...
fn config_configure(config: &mut Config, args: &ArgMatches<'_>) -> CliResult {
